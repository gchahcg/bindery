...
// Wanted page and produces nonsense destination folders.
...
func (s *JunkTitleSignal) Observe(c Candidate, ctx *Context) []models.FilterObservation {
	...
	normalizedTitle := strings.ToLower(strings.TrimSpace(c.Book.Title))
	if normalizedTitle != "" && normalizedTitle != ctx.NormalizedAuthor {
		return nil
	}
